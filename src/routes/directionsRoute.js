...
router.get("/", async (req, res) => {
    try {
        const directionsData = await directions.find();
        const userData = await user.find()
        const weatherData = await weather.find()
        const airQualityData = (await getAirQualityData())
        const finalDirections = getData(directionsData[0].toJSON(), userData[0].toJSON(), weatherData[0].toJSON(), airQualityData.description);
...
function getPossibleDirections(remainingTime, directionsData) {
    ...
    if (remainingTime >= Math.round(directionsData.bicycling.duration_value * 60)) {
...
function getFinalDirection(possibleDirections, airQualityData, weatherData, userData, directionsData) {
    ...
    if (size == 0) {
        ...
        switch (directionsProfile + "-" + typeOfProfile(weatherProfile) + "-" + airQualityData) {
...
function getData(directionsData, userData, weatherData, airQualityData) {
    ...
    const finalDirection = getFinalDirection(possibleDirections, airQualityData, weatherData, userData, directionsData)
